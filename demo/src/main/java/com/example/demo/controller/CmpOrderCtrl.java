...
import com.example.demo.global.DemoGlobals;
...
public class CmpOrderCtrl {
	...
	public String accept(@RequestParam(value = "id") int orderId) {
		...
		order.setState(DemoGlobals.ORDER_ACCEPT);
	...
	public String done(@RequestParam(value = "id") int orderId) {
		OrderInfo order = orderRepo.findById(orderId);
		order.setState(DemoGlobals.ORDER_DONE);
		orderRepo.save(order);
		return "redirect:/cmp/order/list";
	}
	...
	public String getOrderList(HttpSession session,
		...
		Page<OrderInfo> orderPage = orderRepo.findByCmpIdAndStateNot(cmpId, DemoGlobals.ORDER_DONE, pageable);
